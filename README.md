...
PDF変換スクリプトはHTML内の外部画像を自動的にダウンロードしてBase64で埋め込みます。画像が表示されない場合は以下を確認してください:

- HTMLファイルに `<img src="...">` タグが含まれているか確認する
- スクリプトのログに `Warning:` が出力されていないか確認する（画像ダウンロード失敗を示す）
- NotionのS3署名付きURLは一時的なため、ページ取得から時間が経つと期限切れになる場合がある
